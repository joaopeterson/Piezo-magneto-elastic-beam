...
int main()
{
    ...
    // ------------------------------------------- //
    string case_name ("teste");

    double t0 = 0.0;            // initial time
    double t1 = 5000.0;         // final time
    double h  = 5e-2;           // time step
    int Ndt = round((t1-t0)/h); // number of time steps
    int Nss = round(0.9*Ndt);   // Steady-state
    ...
    int N_x = 1200;           // number of points
    int N_xdot = 1200;        // number of points
	...
	f      = 0.083;      // dimensionless excitation amplitude (0.1 -> chaotic, 0.12 -> regular)
    ...
    */
    ...
    /*
     ...
     dimensions: [N_x*N_xdot][3]
     x0[0]  xdot0[0]  z value
     x0[0]  xdot0[1]  z value
      ...     ...       ...
    */
    //double **IC = new double *[N_x*N_xdot];
    //for(int i=0; i<N_x*N_xdot; i++){IC[i] = new double[3];}
    ...
    ofstream ATfile("AT_[" + to_string(int(x0_min)) + "," + to_string(int(x0_max)) +
                    "]_" + "[" + to_string(int(xdot0_min)) + "," + to_string(int(xdot0_max)) + "]_" +
    ...
    ofstream ICfile ("IC_[" + to_string(int(x0_min)) + "," + to_string(int(x0_max)) +
                    "]_" + "[" + to_string(int(xdot0_min)) + "," + to_string(int(xdot0_max)) + "]_" +
                    to_string(N_x) + "X" + to_string(N_xdot) + "_f" + to_string(int(f*1000)) +
                    "_O" + to_string(int(omega*100)) + ".dat", ios::trunc);
    ...
    //for(int i=0; i<N_x*N_xdot; i++){delete [] IC[i];}
